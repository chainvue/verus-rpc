...
| `getwalletinfo.json` | VRSCTEST node | 2026-07-17 | **Two edits, number tokens untouched:** `seedfp` → zeros (same length). This is a genuine scrub: it is wallet-unique and not derivable from the chain (precedent: `registernamecommitment.salt`). The `reserve_balance` currency names → `testcurrency-a..f` is **neutrality, not a privacy control** — the operator's currency name also appears in `test/spend.integration.test.ts`, so it is one grep away in this same public tree |
| `listaddressgroupings.json` | VRSCTEST node | 2026-07-17 | **Truncated 442 groups / 1112 addresses → 2 groups / 3 addresses**, keeping only the structure: a 2-tuple, a 3-tuple (empty `account`), and a value-bearing group. Truncation is for size and noise — **not** a privacy control: `listunspent` and `listaddressgroupings` return only wallet-owned addresses by definition, so common ownership of all 6 testnet addresses across these fixtures is provable from the set itself. Testnet, worthless coins |
