...
| Wire field | Daemon sends | Client gives you |
|---|---|---|
| `balance`, `received`, `satoshis` | satoshi **integers** | `bigint` sats |
| `currencybalance`, `currencyreceived`, `currencyvalues` | **8-decimal** values | `bigint` sats |

Both representations mean the same thing and both become `bigint` sats. A
recorded `getaddressdeltas` fixture carries the same value **twice** —
`satoshis: 1013218` next to `currencyvalues: {…: 0.01013218}` — and the
conformance suite asserts they agree.
